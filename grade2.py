...
# App 1 = questions 1-15
# App 2 = questions 16-30
# App 3 = questions 31-45
# App 4 = questions 46-60
APP_PART = 2
...
def load_questions():
    ...
    try:
        ...
        # Add true global question number: 1, 2, 3 ... 60
        df_all["Global_Question_Number"] = df_all.index + 1
        ...
        return df_part, total_questions
...
# HELPER FUNCTIONS
# =========================================================
def go_to_question(global_question_number):
    """
    Jump using the global question number, e.g. Q16 in app part 2.
    """
    if df is None or len(df) == 0:
        return

    matching_rows = df.index[df["Global_Question_Number"] == global_question_number].tolist()
    ...
    existing = [
        ...
        if r["Question_Number"] == global_question_number
    ]
...
with st.sidebar:
    ...
    if df is not None and len(df) > 0:
        first_q = int(df["Global_Question_Number"].min())
        last_q = int(df["Global_Question_Number"].max())
    ...
    if df is not None and len(df) > 0 and st.session_state.started_review and not st.session_state.completed:
        ...
        available_question_numbers = df["Global_Question_Number"].tolist()

        current_global_q = int(df.iloc[st.session_state.current_q_idx]["Global_Question_Number"])

        selected_q = st.selectbox(
            "Jump to question",
            options=available_question_numbers,
            index=available_question_numbers.index(current_global_q),
    ...
    if df is not None and len(df) > 0 and st.session_state.started_review:
        ...
        if st.session_state.current_q_idx < len(df):
            current_global_q = int(df.iloc[st.session_state.current_q_idx]["Global_Question_Number"])
            st.write(f"Current question: {current_global_q} of {total_questions_all}")
...
# =========================================================
    ...
    <div class="intro-box">
        ...
        <div class="intro-text">
            This is <b>App Part {APP_PART}</b>. You will review <b>Questions {first_q}–{last_q}</b>.
        </div>
        <div class="intro-text">
...
# =========================================================
    ...
    global_q_num = int(row["Global_Question_Number"])
    ...
    <div class="question-box">
        <div class="question-label">Question {global_q_num} of {total_questions_all}</div>
    ...
    hallucination = st.radio(
        ...
        ["No", "Yes"],
        horizontal=True,
        key=f"h_{global_q_num}_{st.session_state.current_ans_idx}"
    )
    ...
    harm_choice = st.radio(
        ...
        index=0,
        horizontal=True,
        key=f"harm_{global_q_num}_{st.session_state.current_ans_idx}"
    )
    ...
    with col1:
        accuracy_val = st.radio(
            ...
            horizontal=True,
            index=3,
            key=f"acc_{global_q_num}_{st.session_state.current_ans_idx}"
        )
    ...
    with col2:
        utility_val = st.radio(
            ...
            horizontal=True,
            index=3,
            key=f"util_{global_q_num}_{st.session_state.current_ans_idx}"
        )
    ...
    with col3:
        detail_val = st.radio(
            ...
            horizontal=True,
            index=3,
            key=f"det_{global_q_num}_{st.session_state.current_ans_idx}"
        )
...
# =========================================================
    ...
    <div class="results-instructions">
        ...
        This is <b>App Part {APP_PART}</b>. Please make sure you download the file before closing this tab.
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.session_state.all_results:
        ...
        wide_df = res_df.pivot(
            index=["App_Part", "Question_Number", "Question"],
        ...
        st.download_button(
            ...
            csv,
            f"medical_eval_export_part_{APP_PART}.csv",
            "text/csv"
        )
